...
CAtomManipMouseDriver::CAtomManipMouseDriver(CMouseHandler* p_handler)
    : CMouseDriver(p_handler)
{
    ...
    SelRequest->SetRequest(GetSelection(),&SH_AtomList,"to manipulate atom positions");
}

//------------------------------------------------------------------------------

CAtomManipMouseDriver::~CAtomManipMouseDriver(void)
{
...
void CAtomManipMouseDriver::MousePressEvent(QMouseEvent* p_event)
{
    if( p_event == NULL ) {
...
void CAtomManipMouseDriver::MouseMoveEvent(QMouseEvent* p_event)
{
    if( p_event == NULL ) {
        ES_ERROR("p_event is NULL");
        return;
...
void CAtomManipMouseDriver::MouseReleaseEvent(QMouseEvent* p_event)
{
    if( p_event == NULL ) {
        ES_ERROR("p_event is NULL");
        return;
...
void CAtomManipMouseDriver::KeyPressEvent(QKeyEvent* p_event)
{
    if( p_event == NULL ) {
        ES_ERROR("p_event is NULL");
        return;
...
void CAtomManipMouseDriver::KeyReleaseEvent(QKeyEvent* p_event)
{
    if( p_event == NULL ) {
...
void CAtomManipMouseDriver::LeaveEvent(QEvent* p_event)
{
    if( p_event == NULL ) {
        INVALID_ARGUMENT("p_event is NULL");
    }
...
void CAtomManipMouseDriver::ResetManipulation(void)
{
    Action = EMA_NOTHING;
    ManipLevel = EML_YZ;
    GetHandler()->EmitOnDriverStatusChanged();
...
CManipulator* CAtomManipMouseDriver::GetManipulator(void)
{
    if( Completed ){
        return(this);
    } else {
...
void CAtomManipMouseDriver::EncodeMouseButtonsPress(QMouseEvent* p_event)
{
    Action = EMA_NOTHING;
...
bool CAtomManipMouseDriver::StartManipulation(void)
{
    ...
    CHistoryNode* p_history = p_mol->BeginChangeWH(EHCL_GEOMETRY,"atom manipulation");
    if( p_history == NULL ){
...
void CAtomManipMouseDriver::Move(const CPoint &dmov,CGraphicsView* p_view)
{
...
void CAtomManipMouseDriver::Rotate(const CPoint &drot,CGraphicsView* p_view)
{
    if( Completed == false ) return;
    if( Allowed == false ) return;
...
void CAtomManipMouseDriver::EndManipulation(void)
{
...
void CAtomManipMouseDriver::RespondToEvent(void)
{
...
void CAtomManipMouseDriver::RequestInitialized(void)
{
    Completed = false;
}
...
void CAtomManipMouseDriver::SelectionChanged(void)
{
    // all atoms must be from the same structure!!!
    CStructure* p_mol = NULL;
    Completed = GetSelection()->NumOfSelectedObjects() >= 1;
    for(int i = 0; i < GetSelection()->NumOfSelectedObjects(); i++){
        CAtom* p_atom = dynamic_cast<CAtom*>(GetSelection()->GetSelectedObject(i));
        if( p_mol == NULL ){
...
void CAtomManipMouseDriver::RequestDetached(void)
{
    Completed = false;
